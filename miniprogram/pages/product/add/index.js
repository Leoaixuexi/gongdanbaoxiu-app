...
Page({
  ...
  async onLoad(query = {}) {
    ...
    if (query.product_code) {
      this.setData({
        'form.product_code': decodeURIComponent(query.product_code),
      });
    }
  ...
  async loadCategories() {
    const options = await dictionary.getOptions('product_category');
    this.setData({
      categories: options || [],
    });
  },
  ...
  async handleSubmit() {
    ...
    try {
      ...
      if (localPhotos.length > 0) {
        ...
        const results = await uploadFiles(localPhotos, 'product');
