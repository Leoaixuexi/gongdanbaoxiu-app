...
async function seedTestData({ user }) {
  ...
  const now = new Date();
  const testItems = [
    { product_id: await getNextId('products'), product_code: 'P-2024-0001', name: 'A4 复印纸', category: '办公耗品', unit: '包', spec: '500张/包 80g', model: 'A4', source: '采购', usage_area: '行政办公室', images: [], stock: 50, min_stock: 10, remark: '常规办公用纸', created_at: now, updated_at: now, created_by: { user_id: user.user_id, name: user.name } },
    { product_id: await getNextId('products'), product_code: 'P-2024-0002', name: '矿泉水 550ml', category: '食品饮料', unit: '箱', spec: '24瓶/箱', model: '', source: '采购', usage_area: '前台接待区', images: [], stock: 30, min_stock: 5, remark: '会议接待用', created_at: now, updated_at: now, created_by: { user_id: user.user_id, name: user.name } },
    { product_id: await getNextId('products'), product_code: 'P-2024-0003', name: '多功能清洁剂', category: '清洁用品', unit: '瓶', spec: '1L', model: '', source: '采购', usage_area: '保洁仓', images: [], stock: 8, min_stock: 10, remark: '低于预警值', created_at: now, updated_at: now, created_by: { user_id: user.user_id, name: user.name } },
    { product_id: await getNextId('products'), product_code: 'P-2024-0004', name: '中性笔 0.5mm', category: '办公耗品', unit: '盒', spec: '12支/盒', model: '黑色', source: '采购', usage_area: '行政办公室', images: [], stock: 0, min_stock: 5, remark: '已缺货，待补货', created_at: now, updated_at: now, created_by: { user_id: user.user_id, name: user.name } },
    { product_id: await getNextId('products'), product_code: 'P-2024-0005', name: '抽纸（200抽）', category: '日用百货', unit: '包', spec: '200抽/包', model: '', source: '采购', usage_area: '办公耗材区', images: [], stock: 100, min_stock: 20, remark: '', created_at: now, updated_at: now, created_by: { user_id: user.user_id, name: user.name } },
  ];
